...
{
    /*
     * DRV8305 SPI: each 16-bit frame both sends SDI and clocks out SDO.
     * A read command (W0=1) returns the requested register data on SDO
     * in the SAME frame. No second dummy frame is needed.
     * SDO top 5 bits are don't care; mask to 11 bits.
     */
    uint16 cmd = (1 << 15) | ((addr & 0xF) << 11) | 0x000;
    return spi_transfer(cmd) & 0x7FF;
}
