...
void DRV8305_Init(void)
{
    ...
    IfxPort_setPinModeInput(PIN_SDO, IfxPort_InputMode_pullDown);
...
{
    uint16 frame = (1 << 15) | ((addr & 0xF) << 11) | 0x000;
    return spi_transfer(frame) & 0x7FF;
}

void DRV8305_TestSPI(void)
{
    DRV8305_WriteReg(0x7, 0x090);
    delay_us(1000);
    uint16 val = DRV8305_ReadReg(0x7);
