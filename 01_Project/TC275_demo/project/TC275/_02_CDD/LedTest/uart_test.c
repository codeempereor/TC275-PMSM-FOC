#include "uart_test.h"
#include "IfxAsclin_reg.h"
#include "IfxPort_reg.h"
#include "Mcal_WdgLib.h"

void UartTest_Init(void)
{
    uint32 iocr;
    uint32 tout;

    Mcal_ResetENDINIT();
    ASCLIN0_CLC.U = 0x00000000u;
    Mcal_SetENDINIT();
    tout = 100000u;
    while((ASCLIN0_CLC.B.DISS != 0u) && (tout > 0u)) tout--;

    ASCLIN0_FRAMECON.U = 0x00000000u;
    ASCLIN0_DATCON.U = 0x00000007u;
    ASCLIN0_BITCON.U = (54u << 0) | (15u << 16);
    ASCLIN0_BRG.U = 0x00000000u;
    ASCLIN0_TXFIFOCON.U = 0x00000000u;
    ASCLIN0_RXFIFOCON.U = 0x00000000u;
    ASCLIN0_IOCR.U = (1u << 31);
    ASCLIN0_FLAGSCLEAR.U = 0xFFFFFFFFu;

    iocr = P14_IOCR0.U;
    iocr &= ~(0xFu << 4);
    iocr |= (0x9u << 4);
    iocr &= ~(0xFu << 12);
    iocr |= (0x2u << 12);
    P14_IOCR0.U = iocr;
}

void UartTest_SendByte(uint8 data)
{
    while ((ASCLIN0_TXFIFOCON.U >> 16) & 0x1Fu);
    ASCLIN0_TXDATA.U = data;
}

void UartTest_SendString(const char* str)
{
    while (*str)
    {
        UartTest_SendByte((uint8)*str++);
    }
}

void UartTest_SendUint32(uint32 val)
{
    char buf[11];
    sint32 i = 0;
    uint32 tmp = val;

    if (val == 0u)
    {
        UartTest_SendByte('0');
        return;
    }

    while (tmp > 0u)
    {
        buf[i++] = (char)('0' + (tmp % 10u));
        tmp /= 10u;
    }

    while (i > 0)
    {
        UartTest_SendByte((uint8)buf[--i]);
    }
}

void UartTest_SendHex(const char* label, uint32 val)
{
    sint32 i;
    uint32 nibble;
    UartTest_SendString(label);
    UartTest_SendString("0x");
    for (i = 7; i >= 0; i--)
    {
        nibble = (val >> (i * 4)) & 0xFu;
        UartTest_SendByte((uint8)(nibble < 10u ? ('0' + nibble) : ('A' + nibble - 10u)));
    }
    UartTest_SendString("\r\n");
}
